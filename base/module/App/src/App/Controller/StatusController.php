...
use App\Model\Entity\StatusEntity;
...
class StatusController extends AbstractController
{
    ...
    public function listAction()
    {
        ...
        $view->setTemplate('/' . VIEW_DIR . '/status/list.phtml');
        return $view;
    }
...
//        $form->get('reset')->setAttribute('value', 'リセット');
        ...
        $view = new ViewModel($values);
        $view->setTemplate('/' . VIEW_DIR . '/status/edit.phtml');
        return $view;
    }
...
//            $this->flashMessenger()->addMessage('指定レコードは存在しません。');
        ...
        if (!$request->isPost()) {
            ...
            $beforeData = make_before_data($row, 'status_no');
            if ($beforeData) {
