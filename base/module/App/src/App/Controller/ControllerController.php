...
use App\Form\ControllerForm;
use App\Filter\ControllerFilter;
...
//        $form->get('submit')->setAttribute('value', '検索');
        ...
        $view->setTemplate('/' . VIEW_DIR . '/controller/list.phtml');
        return $view;
    }
...
//        $form->get('reset')->setAttribute('value', 'リセット');
        ...
        $view = new ViewModel($values);
        $view->setTemplate('/' . VIEW_DIR . '/controller/edit.phtml');
        return $view;
    }
...
//            $this->flashMessenger()->addMessage('指定レコードは存在しません。');
        ...
        if (!$request->isPost()) {
            ...
            $beforeData = make_before_data($row, 'controller_no');
            if ($beforeData) {
