...
use App\Model\Entity\BranchEntity;
...
//        $form->get('submit')->setAttribute('value', '検索');
        ...
        $view->setTemplate('/' . VIEW_DIR . '/branch/list.phtml');
        return $view;
    }
...
//        $form->get('reset')->setAttribute('value', 'リセット');
        ...
        $view = new ViewModel($values);
        $view->setTemplate('/' . VIEW_DIR . '/branch/edit.phtml');
        return $view;
    }

    /**
     * page of update + update processing
     * @return ViewModel
     */
    public function editAction(){
        ...
        // get target information from DB
        $postRequest = $this->container()->get('postRequest');
        if ($postRequest) {
            $this->container()->clear('postRequest');
            $row = get_array_object(json_decode($postRequest, true));
...
//            $this->flashMessenger()->addMessage('指定レコードは存在しません。');
        ...
        if (!$request->isPost()) {
            ...
            $beforeData = make_before_data($row, 'branch_no');
            if ($beforeData) {
